...
The agent CLIs always auto-load their native file from the working directory — that happens regardless of the orchestrator. On top of that, the orchestrator explicitly tells the agent to read `instructionsFile` as its session protocol.
...
AI coding agents run with your permissions. They can read files, execute commands, and access anything your user account can. This creates real risks — whether you use this tool, an IDE copilot, or any agent that can read files and run commands.
...
**Minimize access.** Don't give agents workspace-wide access by default. Block or redact common secret paths (`.env`, `**/secrets*`, `~/.ssh/`, cloud credential directories). The agent should not be able to read anything you wouldn't paste into a chat window.
